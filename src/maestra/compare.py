...
def compare(estimator_a, estimator_b, df: pd.DataFrame, target: str, *, cv: int = 5,
           seeds: int = 1, metric: str | None = None) -> CompareResult:
    ...
    # test_train_ratio = 1/(cv-1): the same per-fold train/val overlap ratio
    # `improves_beyond_noise` uses for a k-fold paired comparison (N1, 2026-07-05).
    ratio = 1.0 / (cv - 1) if cv > 1 else 0.0
    if len(all_deltas) < 2:
    ...
    return CompareResult(
        ...
        metric=metric_name, greater_is_better=greater_is_better, n_folds=cv, n_seeds=seeds,
