...
_SYSTEM_PROMPT = (
    ...
    "free text, NEVER for numeric measurements. A high-cardinality INTEGER column is NOT "
    "automatically an ID either: counts, amounts, populations, years and quantities are "
    "legitimate integer features that can be near-unique. Judge an integer column by what its "
    "NAME and examples say it MEANS -- a running index or record number is a drop, a real "
    "quantity is a keep -- not by cardinality alone. In the profile the field 'id_like'=true "
    "is a deterministic HINT that can over-flag high-cardinality integers; treat it as a "
    "reason to scrutinise the column, not as a verdict. A genuinely unique-per-row TIMESTAMP "
    "or datetime column is a DIFFERENT case from a plain running id: it carries real temporal "
    "signal (seasonality, trend, day-of-week), but only if a later feature-engineering step "
    "gets to decompose it into year/month/weekday -- and that step runs AFTER cleaning, so it "
    "never sees a column you drop here. So do NOT drop a raw timestamp/datetime column just for "
    "being unique per row; KEEP it (the feature engineer's date_parts op parses and replaces it, "
    "so there is no downside to keeping it here). Only drop a datetime column if it is truly "
    "redundant (e.g. exactly duplicates another kept column) or leaks the future. "
